...
def split_windows_args(command: str) -> List[str]:
    # ⚡ Bolt: Fast-path fallback using native .split() when no quotes are present
    # drastically reduces character-by-character parsing overhead.
    if '"' not in command:
        return command.split()
    args: List[str] = []
    current: List[str] = []
    in_quotes = False
    # ⚡ Bolt: Using `for char in command` over a `while` loop with index tracking
    for char in command:
        if char == '"':
